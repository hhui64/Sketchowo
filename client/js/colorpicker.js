...
(function(window, document, undefined) {
    ...
    */
    ...
    ColorPicker.rgb2Hex = function(rgbStr) { // 增强版分割逗号
        let rgb_ = rgbStr.split(',');
        let r_ = parseInt(rgb_[0].split('(')[1]);
        let g_ = parseInt(rgb_[1]);
        let b_ = parseInt(rgb_[2].split(')')[0]);
        let hex_ = "#" + ((1 << 24) + (r_ << 16) + (g_ << 8) + b_).toString(16).slice(1);
        return hex_;
    };
